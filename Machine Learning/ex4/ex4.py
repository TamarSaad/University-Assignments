...
class FinalModel(nn.Module):
    ...
    def __init__(self, image_size):
        ...
        self.batch_norm_fc2 = nn.BatchNorm1d(70)
        self.fc2 = nn.Linear(70, 80)
        self.batch_norm_fc3 = nn.BatchNorm1d(80)
        self.fc3 = nn.Linear(80, 50)
        self.batch_norm_fc4 = nn.BatchNorm1d(50)
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for (data, t) in test_loader:
            output = model(data)
            tag = output.max(1, keepdim=True)[1].item()
...
def main():
    ...
    test_dataset = TensorDataset(tests, torch.zeros(5000))
    test_loader = torch.utils.data.DataLoader(test_dataset, shuffle=False, batch_size=1)

    model = FinalModel(784)
    optimizer = optim.Adam(model.parameters(), lr=eta)

    loss_t, accuracy_t, loss_v, accuracy_v = train(EPOCHS, model, train_loader, "Hello world", optimizer)
