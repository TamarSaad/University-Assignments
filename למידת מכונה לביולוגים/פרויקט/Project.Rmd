...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.show='hold', out.width="70%", out.height="70%"}
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.show='hold', out.width="70%", out.height="70%", echo=FALSE}
#jitter plot
p1 <- ggplot(df_t_s,
       aes(factor(features,
                  levels = c("radius", "texture", "perimeter", "area", "smoothness",
...
```{r, message=FALSE, warning=FALSE, fig.show='hold', out.width="70%", out.height="70%", echo=FALSE}
#jitter plot
p1 <- ggplot(df_t_w,
       aes(factor(features,
                  levels = c("radius", "texture", "perimeter", "area", "smoothness",
...
#minmax_data<-cbind("diagnosis"=clean_data$diagnosis, minmax_data)
str(minmax_data)
```
...
After examination (here and in the coming algorithms), we decided to continue with Z-score since it gave us slightly better results.
...
rpart tree with zscore normalization:
```{r, warning=FALSE, echo=FALSE, out.width="70%", out.height="70%", fig.align='center'}
set.seed(0)
diagnosis = data$diagnosis
cart <- rpart(diagnosis ~ .,
...
```
The results of the normalization methods are the same.

```{r, echo=FALSE}
#split the data to train and test -MINMAX
set.seed(0)
minmax_data$diagnosis<-clean_data$diagnosis
data_split_minmax <- initial_split(minmax_data, prop = 3/4 , strata = diagnosis )
train_m <- training(data_split_minmax)
test_m <- testing(data_split_minmax)
#head(train)
```
...
pred_m_500 <- predict(rf_m_500, data=test_m)$predictions
print("Confusion matrix of Random Forest with 500 trees:")
confusionMatrix(pred_m_500, test_m$diagnosis, positive = "Malignant")


pred_m_1500 <- predict(rf_m_1500, data=test_m)$predictions
print("Confusion matrix of Random Forest with 1500 trees:")
confusionMatrix(pred_m_1500, test_m$diagnosis, positive = "Malignant")


pred_m_900 <- predict(rf_m_900, data=test_m)$predictions
print("Confusion matrix of Random Forest with 900 trees:")
confusionMatrix(pred_m_900, test_m$diagnosis, positive = "Malignant")
...
print("Results of KNN for K = 5:")
CrossTable(x = results_zscore_test_5$truth, y = results_zscore_test_5$.pred_class,
                prop.chisq=FALSE)
print("Results of KNN for K = 9:")
CrossTable(x = results_zscore_test_9$truth, y = results_zscore_test_9$.pred_class, prop.chisq=FALSE)
...
min max:
```{r, warning=FALSE, echo=FALSE, out.width="70%", out.height="70%", fig.align='center'}
knn_m_5 <- nearest_neighbor(neighbors = 5) %>% 
  set_engine('kknn') %>% 
  set_mode('classification') %>%
  fit(diagnosis ~., data = train_m)

results_test_m_5 <- knn_m_5 %>% 
  predict(test_m) %>% 
  mutate(truth = test_m$diagnosis, accuracy = if_else(.pred_class == truth, "yes", "no"))

knn_m_9 <- nearest_neighbor(neighbors = 9) %>% 
  set_engine('kknn') %>% 
  set_mode('classification') %>%
  fit(diagnosis ~., data = train_m)

results_test_m_9 <- knn_m_9 %>% 
  predict(test_m) %>% 
  mutate(truth = test_m$diagnosis, accuracy = if_else(.pred_class == truth, "yes", "no"))

knn_m_15 <- nearest_neighbor(neighbors = 15) %>% 
  set_engine('kknn') %>% 
  set_mode('classification') %>%
  fit(diagnosis ~., data = train_m)

results_test_m_15 <- knn_m_15 %>% 
  predict(test_m) %>% 
  mutate(truth = test_m$diagnosis, accuracy = if_else(.pred_class == truth, "yes", "no"))

results_test_m <- knn_m_5 %>% 
  predict(test_m) %>% 
  mutate(k = "5", 
         truth = test_m$diagnosis) %>% 
          bind_rows(knn_m_9 %>% 
          predict(test_m) %>% 
          mutate(k = "9", truth = test_m$diagnosis)) %>% bind_rows(knn_m_15 %>% 
          predict(test_m) %>% 
          mutate(k = "15", truth = test_m$diagnosis)) %>%
  mutate(accuracy = if_else(.pred_class == truth, "yes", "no"))
...
CrossTable(x = results_test_m_5$truth, y = results_test_m_5$.pred_class, prop.chisq=FALSE)
CrossTable(x = results_test_m_9$truth, y = results_test_m_9$.pred_class, prop.chisq=FALSE)
CrossTable(x = results_test_m_15$truth, y = results_test_m_15$.pred_class, prop.chisq=FALSE)
```
...
As we can see the best K is 15. The results were the same with min-max and with z-score.

Visualize predictions on test split:
*The axes are the two most importance features
We only used the z-score model of k=15, since the 2 models were equally good.
